...
summary(lm2) #This was not presented within the paper, but provided here to display quantitative differences between the two graphs.
```
...
data %>%
  group_by(Treatment) %>%
  summarise(proportion = sum(Larger)/n()) -> proportion_results #relative differences calculation = (absolute diff/mean(number of calls of the two stimuli))
proportion_results
...
I couldn't help but also notice the probability of success values. Here are Hemingway et al's probability of success values calcualted in the binomial test vs. their proportion of choice values calculated from earlier:
...
Although not required, I would have liked to replicate the chi-square differences between models and the quadratic models, but I could not decipher the code in time before submitting this assignment. I think if the statiscal analysis section in the article provided the functions, package or strategy for testing the model comparisons using chi-square tests (and I had another weekend), I might have worked out the replication.
